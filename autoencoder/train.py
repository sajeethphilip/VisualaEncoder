...
def train_model(config):
    """Train the autoencoder model with enhanced validation and monitoring."""
    # Load dataset with validation
    dataset_config = config["dataset"]
    train_dataset, class_counts = load_local_dataset(dataset_config["name"])
    ...
    # Create data loader with the weighted sampler
    ...
    # Training monitoring
    class_correct = {class_name: 0 for class_name in class_counts.keys()}
    class_total = {class_name: 0 for class_name in class_counts.keys()}
    ...
    for epoch in range(config["training"]["epochs"]):
        ...
        for batch_idx, (images, targets) in enumerate(progress_bar):
            ...
            if hasattr(train_dataset, 'imgs'):
                ...
                full_paths = [train_dataset.samples[i][0] for i in range(len(images))]
